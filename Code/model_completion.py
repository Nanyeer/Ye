...
parser.add_argument('--resume-dir',
                    default='E:/hjl/label-inference-attacks-main/Code/saved_experiment_results/saved_models/',
...
checkpoint_dir = os.path.join(args.resume_dir, f'{args.dataset_name}_saved_models')
args.resume = os.path.join(checkpoint_dir, args.resume_name)
args.out = checkpoint_dir
...
def main():
    ...
    print(best_acc)
# ===== 在这里追加：把 MC 攻击 ASR 写回对应的框架 txt =====
    # train_acc 存在 best_acc 里，test_acc 在 test_accs 最后一个
    train_asr = best_acc
    test_asr = test_accs[-1] if len(test_accs) > 0 else None

    # 从 args.resume 推出对应的训练日志 txt 文件名
    # 例如：E:/.../CIFAR10_saved_framework_lr-0.1_normal_half-16.pth
    # 对应：E:/.../CIFAR10_saved_framework_lr-0.1_normal_half-16.txt
    framework_txt = None
    if args.resume.endswith(".pth"):
        base = os.path.basename(args.resume)
        txt_name = os.path.splitext(base)[0] + ".txt"
        framework_txt = os.path.join(os.path.dirname(args.resume), txt_name)

    if framework_txt is not None:
        try:
            with open(framework_txt, "a", encoding="utf-8") as f:
                f.write("\n\n=== Model Completion Label Inference Attack ===\n")
                f.write(f"Dataset: {args.dataset_name}\n")
                f.write(f"Checkpoint used for MC: {os.path.basename(args.resume)}\n")
                f.write(f"MC Train ASR (top-1): {train_asr:.4f}\n")
                if test_asr is not None:
                    f.write(f"MC Test  ASR (top-1): {test_asr:.4f}\n")
                f.write("===============================================\n")
        except Exception as e:
            print("Warning: failed to append ASR to framework txt:", framework_txt, "Error:", e)

    # 同时在终端打印 ASR，方便直接看到
    print("\n=== Model Completion Label Inference Attack ===")
    print(f"Training set ASR (top-1): {train_asr:.4f}")
    if test_asr is not None:
        print(f"Test set ASR (top-1): {test_asr:.4f}")
    print("===============================================\n")
...
if __name__ == '__main__':
    ...
    if args.print_to_txt == 0:
        ...
        out_dir = args.out   # 即 E:/hjl/.../Code/saved_experiment_results/saved_models/CIFAR10_saved_models/
