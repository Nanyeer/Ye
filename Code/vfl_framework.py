...
from possible_defenses import KSSController, KFMController
...
class VflFramework(nn.Module):
    ...
    def __init__(self):
        ...
        # ---- KSS / KFM controllers (new) ----
        self.kss_ctrl = KSSController(
            pmin=args.kss_pmin,
            pmax=args.kss_pmax,
            seed=args.kss_seed) if args.kss else None

        self.kfm_ctrl = KFMController(
            beta=args.kfm_beta,
            seed=args.kfm_seed) if args.kfm else None
    ...
    def simulate_train_round_per_batch(self, data, target):
        ...
        # defenses here: server‑side defenses on gradients sent to bottom models
        ...
        if self.defense_ppdl:
            possible_defenses.dp_gc_ppdl(
                epsilon=1.8, sensitivity=1, layer_grad_list=[grad_output_bottom_model_a],
                theta_u=args.ppdl_theta_u, gamma=0.001, tau=0.0001)
            possible_defenses.dp_gc_ppdl(
                epsilon=1.8, sensitivity=1, layer_grad_list=[grad_output_bottom_model_b],
                theta_u=args.ppdl_theta_u, gamma=0.001, tau=0.0001)
        ...
        # -------- KSS / KFM on active party gradient (new) --------
        if self.kss_ctrl is not None or self.kfm_ctrl is not None:
            # 使用当前 batch 的主动方梯度列表（grad_output_bottom_model_a: (B,D)）
            per_sample_grads = grad_output_bottom_model_a.detach()

            # 普通平均梯度
            g_active = per_sample_grads.mean(dim=0)

            # KSS: 秘密子采样 + 无偏重加权
            if self.kss_ctrl is not None:
                g_active = self.kss_ctrl.subsample_and_reweight(
            ...
            # KFM: 基于被动方特征均值的特征级掩码
            if self.kfm_ctrl is not None:
                with torch.no_grad():
                    feat_mean_enc = output_tensor_bottom_model_b.detach().mean(dim=0)
                g_active = self.kfm_ctrl.mask(
                    g_mean=g_active,
                    feat_mean_enc=feat_mean_enc,
                    epoch=current_epoch,
                    batch_id=current_batch_idx
                )

            # 用防御后的梯度替换原有梯度
            grad_output_bottom_model_a = g_active.unsqueeze(0).expand_as(grad_output_bottom_model_a)

        # server sends back grad_output_bottom_model_a to the adversary (participant a)
...
def test_per_epoch(test_loader, framework, k=5, loss_func_top_model=None):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            if args.use_top_model:
                output_framework = framework.top_model(
            ...
            correct_top1_batch, correct_topk_batch = correct_counter(
            ...
            if int(0.1 * len(test_loader)) > 0:
                if count % int(0.1 * len(test_loader)) == 0 and \
...
def main():
    global current_epoch, current_batch_idx
        ...
        if epoch == args.epochs - 1:
            ...
            with open(dir_save_model + '/' + txt_name + '.txt', 'w+') as file:
                ...
                if not args.use_top_model:
                    ...
                    print("Direct label inference accuracy:",
            ...
            torch.save(
                model,
                os.path.join(dir_save_model, f"{args.dataset}_saved_framework{setting_str}.pth"),
                pickle_module=dill)
            ...
            if args.if_cluster_outputsA:
                ...
                for i in range(num_classes):
                    plt.scatter(df_outputsA_pca_tsne.loc[i][0],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test-upper-bound',
    ...
    parser.add_argument('--ppdl-theta-u',
                        help='theta-u parameter for defense privacy-preserving deep learning',
                        type=float, default=0.75)
    parser.add_argument('--gc-preserved-percent',
    ...
    parser.add_argument('--multistep_grad_bins',
                        help='number of bins in multistep-grad',
                        type=int, default=6)
    parser.add_argument('--multistep_grad_bound_abs',
                        help='bound of multistep-grad',
                        type=float, default=3e-2)

    # KSS / KFM (new)
    parser.add_argument('--kss', action='store_true',
                        help='use key‑controlled secret subsampling on active party')
    parser.add_argument('--kss-pmin', type=float, default=0.2)
    parser.add_argument('--kss-pmax', type=float, default=0.8)
    parser.add_argument('--kss-seed', type=int, default=1234)

    parser.add_argument('--kfm', action='store_true',
                        help='use key‑controlled feature‑level masking on active party')
    parser.add_argument('--kfm-beta', type=float, default=0.5,
                        help='masking strength β in [0,1]')
    parser.add_argument('--kfm-seed', type=int, default=5678)
